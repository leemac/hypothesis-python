...
import hypothesis._lifecycle as lifecycle
...
def hook_executor(self):
    ...
    setup = lifecycle.get_hook(definition, 'setup_example')
    teardown = lifecycle.get_hook(definition, 'teardown_example')
    execute_example_output = lifecycle.get_hook(
        definition, 'execute_example_output')

    def execute(data, function):
        try:
            setup(self)
            result = function(data)
            return execute_example_output(self, result)
        finally:
            teardown(self)
    return execute
...
def new_style_executor(runner):
    ...
    if lifecycle.has_hooks(type(runner)):
        return hook_executor(runner)
